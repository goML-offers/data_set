...
def generate_text_headline(cluster_description, categorical_description, asset_to_individulize_file, tone_of_voice):
    ...
    profiling = openai.ChatCompletion.create(
        # model="gpt-3.5-turbo-0613",
        model= "gpt-4",
        # model = "gpt-3.5-turbo-16k",
        # model="gpt-4-0613",
    ...
    print(customer_profile_summary)
    ...
    Also give me a two to three words headline as well. It should be in a json format having two keys summary and headline only.
    ...
    llm_json_dumps = json.dumps(llm_answer)
    print(type(llm_json_dumps), llm_json_dumps)
    llm_json = json.loads(llm_json_dumps)
...
def combine_text_image(cluster_id, background_image_path, logo_path, asset_to_individualize, primary_color_rgb, secondary_color_rgb):
    ...
    # Set the text to be displayed
    formatted_text = format_text(asset_to_individualize, 10)  # Adjust the max_words_per_line as needed
    text = formatted_text
    ...
    draw.text(text_position_primary, text, fill=primary_color_rgb, font=ImageFont.load_default())
...
def combine_text_image_headline(cluster_id, background_image_path, logo_path, asset_to_individualize, primary_color_rgb, secondary_color_rgb):
    font_size =30
    width, height = 500, 400
    base_image = Image.open(background_image_path)
    base_image = base_image.resize((width, height))
...
def asset_creation(table_name, user_id, project_id, extraction_id_brand, extraction_id_tone, file_id_log, file_id_background_image):
    ...
    try:
        ...
        type = "picture_bank"
        user = supabase.from_("file_data").select("*").eq("id",file_id_background_image).execute()
        print("userrrrr", user)
        user_data = [record for record in user.data]
        print("user_dataaaaa",user_data)
        data = json.dumps(user_data)
        d = json.loads(data)


        file_type = d[0]['type']
        print("file_typeeeee", file_type)
        ...
        #     text = "Empower Your Legacy Giants with our premier solutions. Captivating 8,200+ financial industries and counting, our robust marketing tools are uniquely designed to serve your distinct needs. Embrace efficiency, cultivate growth and be a part of the top-financial trendsetters across the United Kingdom. Propel your business forward in a landscape dominated by Kingsley Napley LLP and others. Join the movement - Experience difference with us."
            ...
            # Draw text in primary color with default font
            draw.text(text_position_primary, text, fill=primary_color_rgb, font=primary_fo"""
